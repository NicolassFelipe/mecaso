...
import { db } from "@/src/db"; // <-- 1. Importamos nuestra conexión a la BD
import { proveedores } from "@/src/db/schema"; // <-- 2. Importamos la tabla

// Fíjate que ahora agregamos la palabra "async" a la función.
// Esto convierte a nuestro componente en un "React Server Component".
export default async function FeaturedVendors() {
  
  // 3. ¡LA MAGIA! Le decimos a Drizzle que traiga todos los proveedores de la BD
  const dbVendors = await db.select().from(proveedores);
    ...
    <section className="bg-zinc-50 py-20 border-t border-zinc-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {/* 4. Usamos la lista que vino de la base de datos (dbVendors) */}
          {dbVendors.map((vendor) => (
            <Link key={vendor.id} href={`/proveedor/${vendor.slug}`} className="group block h-full">
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
                ...
                <div className="relative h-48 w-full">
                  {/* Usamos imagenUrl desde la BD, y si no hay, ponemos una genérica */}
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img 
                    src={vendor.imagenUrl || "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?q=80&w=800&auto=format&fit=crop"} 
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center gap-1 mb-4">
                    ...
                    <span className="font-bold text-zinc-800 text-sm">{vendor.rating}</span>
